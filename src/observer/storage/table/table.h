...
public:
   ...
   * @param attributes 字段
   */
  RC create(Db *db, int32_t table_id, const char *path, const char *name, const char *base_dir,std::span<const AttrInfoSqlNode> attributes);
  
  RC destroy(const char* dir);  //在Table类当中添加一个destroy接口，供db.cpp里的drop_table调用
