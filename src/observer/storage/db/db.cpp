...
RC Db::drop_table(const char *table_name){
  RC rc=RC::SUCCESS;
  auto itera=opened_tables_.find(string(table_name));
  if(itera==opened_tables_.end()) rc=RC::SCHEMA_TABLE_NOT_EXIST;
  else{
    Table* table=itera->second;
    RC destory_check = table->destroy(path_.c_str()); // 让表自己销毁资源
    if(destory_check==RC::SUCCESS) opened_tables_.erase(itera); // 删除成功的话，从表list中将它删除
    else rc=destory_check;
    delete table;
  }
  return rc;
}
